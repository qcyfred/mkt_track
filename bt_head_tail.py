...
df = pd.read_sql(sql, engine)
df = df.pivot(columns='sec_code', index='trade_date', values='value_60')
all_sec_codes = df.columns
rank_df = df.rank(axis=1)
max_rank_df = rank_df[rank_df > 45]  # 超涨
min_rank_df = rank_df[rank_df < 5]  # 超跌

pos_multiplier_df = pd.DataFrame(np.ones(df.shape),
                                 columns=df.columns,
                                 index=df.index)
pos_multiplier_df[max_rank_df.notnull()] = 0  # 做空
pos_multiplier_df[min_rank_df.notnull()] = 2  # 做多
...
# TODO: 权重shift(-1)，每天估算申赎清单
# origin_pos_df = origin_pos_df.shift(-1)
